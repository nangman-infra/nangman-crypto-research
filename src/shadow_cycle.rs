...
pub fn build_shadow_cycle_decision(
    ...
    for run in shadow_runs {
        run_identity_parts.push(run.shadow_validation_run_id.as_str());
        all_symbols.insert(run.symbol_canonical.clone());
    ...
    run_identity_parts.sort_unstable();
    let candidate_lifecycle_count = candidates.len();
    let target_waiting_count = candidates
        .values()
        .filter(|state| state.target_materialized_count == 0 && state.observed_count > 0)
        .count();
    let partially_materialized_count = candidates
        .values()
        .filter(|state| {
        ...
        .count();
    let pending_target_window_candidate_count = candidates
        .values()
        .filter(|state| state.pending_target_count > 0)
        .count();
    let sample_ready_count = candidates
        .values()
        .filter(|state| state.sample_requirement_met())
        .count();
    let deficient_count = candidates
        .values()
    ...
    let pending_count = candidates
        ...
        .count();
    let total_sample_deficit = candidates
        .values()
        .map(CandidateShadowState::sample_deficit)
        .sum();
    let next_observation_not_before_ms = candidates.values().fold(None, |current, state| {
        min_optional_ms(current, state.next_pending_target_deadline_ms)
    });

    let (source_verdict, scheduler_action) = if shadow_runs.is_empty() {
        ("NO_SHADOW_CANDIDATES", ShadowCycleSchedulerAction::Noop)
    } else if latest_l1_as_of_ms.is_none() {
        (
    ...
    ShadowCycleDecision {
        ...
        shadow_sample_state: ShadowCycleSampleState {
            ...
            target_window_materialized_count: target_materialized_count,
